...
__global__ void max_kernel(const float* image, std::uint32_t width, std::uint32_t height) {
    ...
    if (x < width && y < height) {
        ...
        atomicMax((int*)d_max_val, __float_as_int(pixel_value));
    }
}
...
double get_max_value(void** d_source_image, std::uint32_t source_image_height, std::uint32_t source_image_width){
    ...
    find_max_kernel<<<blocksperGrid, threadsPerBlock>>>(d_img, source_image_width, source_image_height, d_max_result);
