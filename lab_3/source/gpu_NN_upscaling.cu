...
__global__ void nn_upscale_kernel(const double* source, double* result, std::uint32_t source_width, std::uint32_t width, std::uint32_t height) {
    ...
    if (x < width && y < height) {
        ...
        result[y * width + x] = source[in_y * source_width + in_x];
    }
}
