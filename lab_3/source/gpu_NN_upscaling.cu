...
__global__ void nn_upscale_kernel(double* source, double resul, double length, uint32_t width, uint32_t height) {
...
void NN_image_upscaling(void** d_source_image, std::uint32_t source_image_height, std::uint32_t source_image_width, void** d_result){
    uint32_t width = get_NN_upscaled_width(source_image_width);
    uint32_t height = get_NN_upscaled_height(source_image_height);
    size_t size_ende = width*height * sizeof(double);

    cudaMalloc(d_result, size_ende);

    auto source = reinterpret_cast<const double *>(d_source_image);
    double* result = static_cast<double*>(cudaMalloc(d_result));
